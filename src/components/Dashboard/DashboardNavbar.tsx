...
export default function DashboardNavbar() {
    ...
    return (
        <>
            ...
            <div className="fixed bottom-0 left-0 right-0 flex justify-center z-50 bg-base-100 shadow-lg py-2 sm:py-3 md:py-4 overflow-x-auto">
                <div role="tablist" className="tabs tabs-box gap-1 sm:gap-2 flex-nowrap">
                    {tabs.map((tab) => (
                        <Link
                            ...
                            className={`tab mx-1 sm:mx-3 md:mx-8 px-2 sm:px-4 md:px-6 py-2 my-2 flex items-center justify-center whitespace-nowrap ${isActive(tab.path) ? 'tab-active' : ''}`}
                        >
                            {tab.icon}
                        </Link>
                    ))}
